...
Version 4 adds computation and starts to fill in the transformer block.
"""
import os
import pickle
import numpy as np
...
class SimpleLanguageModel(nn.Module):
    def __init__(self, vocab_size, n_embed):
        ...
        # We are going to add multiple self-attention heads.  # Note that we are dividing up the
        # embedding space among the heads.  So if there are 4 heads, then each head will work
        # on n_embed/4 = 8 dimensions of the embedding space.  They will get catted together
        # at the end to get back to the full 32 dimension space.
        self.sa_heads = MultiHeadAttention(4, n_embed//4)
        # After computing attention, we will now add some computation.
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.sa_heads(x)
        # Now the model can "think" about what was learned from the self attention.  This
        # operates at the token level, no communication between them.
        x = self.ffwd(x)  # (B, T, C)
